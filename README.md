...
Публикационный пример REST API на Express, MongoDB и JWT. Проект демонстрирует
регистрацию и авторизацию пользователей, защищённые операции со статьями и
...
Приложение построено как небольшой модульный монолит: `index.js` отвечает за
подключение к базе данных и жизненный цикл HTTP-сервера, `app.js` собирает
маршруты Express, контроллеры содержат прикладную логику, модели описывают
коллекции MongoDB.

## Переменные окружения

Секреты хранятся только в локальном `.env.local` и не попадают в Git или Docker-
образ. Начните с шаблона:
...
Compose запускает MongoDB во внутренней сети без публикации её порта наружу.
Приложение ждёт успешный healthcheck базы данных, запускается от
непривилегированного пользователя и сохраняет данные MongoDB в именованном
Docker-томе.
...
Приложение загружает `.env.local` раньше `.env`. При отсутствии обязательных
переменных запуск завершается с понятным сообщением, не создавая подключение с
...
Основные поля регистрации: `email`, `password`, `fullName`, необязательное
`avatarUrl`. Для статьи используются `title`, `text`, `tags` и необязательное
`imageUrl`.
